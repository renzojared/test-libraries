using GenerateDocument.Sample.Components;
using GenerateDocument.Sample.Entities;

namespace GenerateDocument.Sample.Documents;
    ...
    void ComposeTable(IContainer cnt)
    {
        cnt.Table(tbl =>
        {
            ...
            tbl.Header(header =>
            {
                ...
                {
                    var retrn = cnt.DefaultTextStyle(x => x.SemiBold()).PaddingVertical(5).BorderBottom(1)
            ...
            foreach (var item in ModelGeneric.Items)
            {
                ...
                tbl.Cell().Element(CellStyle).AlignRight().Text(item.Quantity.ToString());
